...
.PHONY: all setup data model features tables run sweep web clean
...
run:              ## score every draft with the best validated model and all pre-draft signals
	PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True $(PY) -m validation.run --models stack+momentum --protocols causal --tag published

run-all-models:   ## same, plus the stack's members and the ridge / LightGBM baselines and the pooled diagnostic (~10 min)
	PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True $(PY) -m validation.run --models stack,stack+market,tabfm,tabfm_cls,ridge,lgbm --tag published_full

sweep:            ## the tournament: audit, every feature group x model on validation years across all GPUs, layer-2 rules, report
