...
from nbadraft.config import OUT

DB = OUT / "results.sqlite"

SCHEMA = """
CREATE TABLE runs (run_id TEXT PRIMARY KEY, created TEXT, tag TEXT, models TEXT, redraft_model TEXT, north_star TEXT, target TEXT,
...
CREATE TABLE redraft_picks (run_id TEXT, year INTEGER, model TEXT, protocol TEXT, player TEXT, bbref_id TEXT, actual_pick INTEGER, new_pick INTEGER,
                            team TEXT, college TEXT, modelled INTEGER, pred REAL, peak_war REAL, seasons_played INTEGER, labelled INTEGER);
...
def rebuild():
    ...
    for path in sorted((OUT / "runs").glob("*/run.json")):
        ...
        con.execute("INSERT INTO runs VALUES (?,?,?,?,?,?,?,?,?,?,?)",
                    (r["run_id"], r["created"], r["tag"], ",".join(r["models"]), r["redraft_model"], r["north_star"], r["target"],
        ...
        con.executemany("INSERT INTO redraft_picks VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                        [(r["run_id"], d["year"], d["model"], d["protocol"], p["player"], p["bbref_id"], p["actual_pick"], p["new_pick"], p["team"],
                          p["college"], int(p["modelled"]), p["pred"], p["peak_war"], p["seasons_played"], int(p["labelled"]))
